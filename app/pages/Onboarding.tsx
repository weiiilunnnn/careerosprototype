  ...
  useEffect(() => {
    ...
    function buildCircuits() {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        circuits.push({
          ...
          color: Math.random() > 0.5 ? primaryColor : gridColor,
        });
      }
    }
    ...
    function draw() {
      ...
      for (let r = 0; r < rows; r += 4) {
        ...
        ctx.moveTo(0, gy);
        ctx.lineTo(W, gy);
        ctx.strokeStyle = `${gridColor}0.035)`;
        ctx.stroke();
      }
      ...
      streaks.forEach((s) => {
        ...
        gradient.addColorStop(
          0.5,
          dark ? `hsla(${s.hue},80%,65%,${s.alpha})` : `hsla(${s.hue},70%,55%,${s.alpha})`
        );
      ...
      hexRings.forEach((h) => {
        ...
        drawHex(hx, hy, h.size);
        ctx.stroke();

        ctx.strokeStyle = `${primaryColor}${h.opacity * 0.5})`;
        drawHex(hx, hy, h.size * 0.6);
        ctx.stroke();
      });
      ...
      circuits.forEach((c) => {
        ...
        for (let i = 1; i < c.points.length; i++) {
          ...
          if (drawn + segLen < target) {
            ...
            ctx.beginPath();
            ctx.arc(ox1, y1, 2.5, 0, Math.PI * 2);
            ctx.fillStyle = `${c.color}${c.opacity * 1.5})`;
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.moveTo(ox0, y0);
            ctx.lineTo(ox0 + (ox1 - ox0) * frac, y0 + (y1 - y0) * frac);
            ctx.strokeStyle = `${c.color}${c.opacity * 1.4})`;
            ctx.stroke();

            const tx = ox0 + (ox1 - ox0) * frac;
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `${primaryColor}${alpha.toFixed(2)})`;
        ctx.fill();
      });
        ...
        particles.forEach((p, i) => {
          for (let j = i + 1; j < particles.length; j++) {
            ...
            if (dist < 110) {
              ...
              ctx.lineTo(qx, q.y);
              ctx.lineWidth = 0.5;
              ctx.strokeStyle = `${primaryColor}${((1 - dist / 110) * 0.12).toFixed(3)})`;
              ctx.stroke();
            }
          }
        });
...
const steps: Step[] = [
    ...
    fields: [
      {
        ...
        label: "Resume or CV",
        type: "file",
        helper: "Upload one or more PDF, DOCX, or image files",
      },
      {
        name: "certificates",
        label: "Certificates",
        type: "file",
        helper: "Upload one or more certificates or achievement proofs",
      },
      {
        name: "portfolio",
        label: "Portfolio or GitHub link",
        type: "text",
  ...
  },
  {
    key: "profile",
    label: "Profile",
    title: "Finish setting up your candidate profile.",
    description: "Add your contact and login details so employers can reach you and your CareerOS account is ready.",
    icon: UserRound,
    fields: [
      { name: "fullName", label: "Full name", placeholder: "Jason Tan" },
      { name: "email", label: "Email", placeholder: "yourname@email.com" },
      { name: "phone", label: "Phone number optional", placeholder: "+60 12-345 6789" },
  ...
  return (
    <div className="block">
      <div className="mb-2 flex items-center justify-between gap-3">
        <span className="block text-sm font-bold" style={{ color: theme.deepNavy }}>
          {field.label}
        </span>
        ...
        <button
          type="button"
          onClick={addItem}
          className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-extrabold text-white shadow-sm transition hover:scale-[1.02]"
          ...
          <div key={`${field.name}-${index}`} className="flex gap-2">
            ...
            {values.length > 1 && (
              <button
                ...
                className="flex w-11 shrink-0 cursor-pointer items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200"
              >
                <X className="h-4 w-4" />
              </button>
            )}
...
function FileUploadField({
  ...
  return (
    <div className="block">
      <span className="mb-2 block text-sm font-bold" style={{ color: theme.deepNavy }}>
        {field.label}
      </span>

      <div
        className="flex items-center justify-between gap-4 rounded-xl border bg-white/80 px-4 py-3 backdrop-blur-sm"
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-bold" style={{ color: theme.deepNavy }}>
              {fileSummary}
            </p>
        ...
        <label
          htmlFor={inputId}
          className="shrink-0 cursor-pointer rounded-full px-4 py-2 text-sm font-extrabold text-white transition hover:scale-[1.02]"
          style={{ background: theme.rose2 }}
        >
          Upload
        </label>
      ...
      {files.length > 0 && (
        <div className="mt-3 space-y-2">
          {files.map((fileName, index) => (
            <div
              key={`${fileName}-${index}`}
              className="flex items-center justify-between gap-3 rounded-xl border bg-white/80 px-4 py-2"
              style={{ borderColor: theme.border }}
            >
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition hover:bg-slate-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
      ...
      {steps.map((step, index) => (
        <div
          ...
          style={{
            ...
            background: current === index ? theme.rose2 : theme.border,
          }}
        />
      ))}
    </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(255,242,246,0.55) 0%, transparent 70%)",
        }}
      />

      <div className="relative mx-auto flex min-h-[calc(100vh-80px)] max-w-7xl items-center px-5 py-16 sm:px-8">
        <div
          className="w-full overflow-hidden rounded-[2.25rem] shadow-[0_32px_90px_rgba(19,31,51,0.2)]"
          style={{
            background: "linear-gradient(160deg,#0a1628 0%,#0f1f3a 40%,#12162e 100%)",
          }}
        >
          <div className="relative min-h-[520px] overflow-hidden">
            ...
            <div className="relative z-10 flex min-h-[520px] flex-col items-start justify-center px-10 sm:px-16 lg:px-20">
              ...
              >
                <div
                  className="mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-2 text-[11px] font-extrabold uppercase tracking-[.22em]"
                  style={{
                    color: theme.rose1,
                    backgroundColor: "rgba(240,77,122,.1)",
                    borderColor: "rgba(240,77,122,.25)",
                  }}
                >
                ...
                <h1 className="text-5xl font-black leading-[1.02] tracking-tight text-white sm:text-6xl lg:text-7xl">
                  ...
                  <span
                    style={{
                      background: `linear-gradient(135deg,${theme.rose2},${theme.rose1})`,
                ...
                <button
                  ...
                  style={{
                    backgroundColor: theme.rose2,
                    boxShadow: "0 8px 32px rgba(224,0,70,.35)",
                  }}
                >
                  Start Onboarding
  ...
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(255,242,246,0.45) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-6 sm:px-8">
        ...
        >
          <div
            className="grid overflow-hidden rounded-[2rem] shadow-[0_26px_80px_rgba(19,31,51,0.15)]"
            style={{
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 flex min-h-full flex-col p-8">
                ...
                <div
                  className="mb-2 inline-flex w-fit items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.22em]"
                  style={{
                    color: theme.rose1,
                    backgroundColor: "rgba(240,77,122,.1)",
                    borderColor: "rgba(240,77,122,.25)",
                ...
                <div className="mt-auto rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur-md">
                  ...
                  {completedSteps.length === 0 ? (
                    <p className="text-sm font-medium" style={{ color: "rgba(180,200,230,.5)" }}>
                    ...
                    {steps.map((s, i) => (
                      <div
                        ...
                        style={{
                          ...
                          background: i <= stepIndex ? theme.rose2 : "rgba(255,255,255,.2)",
                        }}
                      />
                    ))}
                  </div>
            ...
            <div className="flex flex-col bg-white/90 p-7 backdrop-blur-sm">
              <div className="mb-5 shrink-0">
                <h3 className="text-3xl font-black" style={{ color: theme.deepNavy }}>
                  {step.label}
                </h3>

                <p className="mt-1 text-sm font-medium" style={{ color: theme.muted }}>
                  {step.key === "profile"
                    ? "Complete your profile and account details to finish your first Living Portfolio."
                    ...
                    <label key={field.name} className="block">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full rounded-xl border bg-white/80 px-4 py-3.5 text-sm font-medium outline-none transition placeholder:text-slate-400 backdrop-blur-sm focus:ring-1 focus:ring-[#E00046]/30"
                          style={{
                            borderColor: theme.border,
                            color: theme.deepNavy,
                          }}
                          onFocus={(e) => {
                            e.target.style.borderColor = theme.rose2;
                          }}
                          onBlur={(e) => {
                ...
                <div className="flex gap-3">
                  ...
                  {stepIndex === steps.length - 1 ? (
                    <button
                      ...
                      style={{
                        background: theme.rose3,
                        boxShadow: "0 8px 28px rgba(224,0,70,.28)",
                      }}
                    >
                      Create User Profile
                      <ArrowRight className="h-5 w-5" />
...
const creationStages = [
  {
    ...
    detail: "Organising your education, skills, experience & profile details",
    duration: 2200,
  },
  {
    label: "Calculating career paths",
    detail: "Mapping your profile against 400+ career routes",
    duration: 2600,
  },
  {
    label: "Matching best careers & jobs",
    detail: "Finding the roles that fit you right now",
    duration: 2000,
  },
];
  ...
  useEffect(() => {
    ...
    async function run() {
      for (let i = 0; i < creationStages.length; i++) {
        ...
        setStageIndex(i);
        await new Promise((resolve) => setTimeout(resolve, creationStages[i].duration));
  ...
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background: "radial-gradient(ellipse 60% 55% at 50% 50%, rgba(224,0,70,0.13) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="relative z-10 flex w-full max-w-lg flex-col items-center px-6 py-16 text-center">
            ...
            {allDone ? (
              ...
              >
                ...
                <polyline
                  ...
                  style={{
                    animation: "draw-check .5s ease forwards .1s",
                    strokeDasharray: 38,
                    strokeDashoffset: 38,
                  }}
                />
              ...
              >
                {isActive && (
                  <div
                    className="pointer-events-none absolute inset-0"
                    style={{
                      background: "linear-gradient(90deg,transparent,rgba(240,77,122,0.07),transparent)",
                      animation: "slide-x 1.6s ease infinite",
                    }}
                  />
                )}
                ...
                <div className="relative flex items-center gap-4">
                  <div
                    ...
                    style={{
                      ...
                      border: isDone
                        ? "none"
                        : `1.5px solid ${isActive ? "rgba(240,77,122,0.4)" : "rgba(255,255,255,0.1)"}`,
                    }}
                  >
                    {isDone ? (
                      <svg
                  ...
                  <div className="min-w-0 flex-1">
                    <p
                      ...
                      style={{
                        color: isDone ? theme.rose1 : isActive ? "white" : "rgba(180,200,230,0.45)",
                      }}
                    >
                      {stage.label}
                    </p>
                    ...
                    <div className="flex shrink-0 gap-1">
                      {[0, 1, 2].map((dot) => (
                        <div
                          ...
                          style={{
                            background: theme.rose2,
                            animation: `bounce-dot .9s ease-in-out ${dot * 0.15}s infinite`,
                          }}
                        />
                      ))}
                    </div>
        ...
        >
          ...
          <p className="mt-1 text-sm font-medium" style={{ color: "rgba(180,200,230,0.65)" }}>
            Taking you to your Best Jobs…
          </p>
        </div>
      </div>
    </div>
...
export default function Onboarding() {
  ...
  const goBack = () => {
    setStepIndex((old) => Math.max(0, old - 1));
  };
  ...
  return (
    ...
    >
      ...
      {screen === "creating" && (
        <CreatingProfile
          onDone={() => {
            window.location.href = "/?view=career-landscape";
          }}
        />
      )}
    </main>
