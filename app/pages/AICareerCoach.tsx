...
function buildCoachReply(input: string) {
  ...
  if (normalized.includes("grab")) {
    return "For Grab, I would prepare you for a product operations, marketplace analyst, or business operations pathway. Based on your CareerOS profile, the strongest angle is analytics plus practical business problem solving. The main gaps to close are: one Grab-style marketplace case study, clearer SQL or dashboard evidence, and interview stories that show speed, judgement, and customer impact. Build a case around ride or delivery demand: define the problem, choose metrics such as cancellation rate, conversion, fulfilment time, or driver supply, analyse the pattern, then recommend one action. For referrals, CareerOS would check alumni, past colleagues, and second-degree contacts connected to Grab before you apply. If you have a past employee signal, reach out with your case study first and ask whether the target team values this kind of evidence. For screening, prepare SQL joins and aggregations, a marketplace metrics explanation, one product/operations case, and STAR stories for teamwork, ambiguity, and fixing a failed process. Your next move: finish the case study, update the first three portfolio artefacts, then ask one connection for role-fit feedback before requesting referral support.";
  }

  if (normalized.includes("tell me about") && (normalized.includes("prepare") || normalized.includes("careeross") || normalized.includes("careeros"))) {
    return "I would treat this as a target-readiness plan, not a simple profile summary. First, CareerOS compares your stored portfolio, education, work history, and connection graph against people already studying or working at that target. Then it separates the plan into five parts: requirements, evidence gaps, connections, screening or interview preparation, and next action timing. Your strongest move is to create one target-specific proof item, make the first three portfolio artefacts match the target, check whether any alumni, classmates, former teammates, mentors, or second-degree contacts are connected there, and prepare the likely screening format before applying. If this is an employer, that may include LeetCode, SQL, case interviews, cloud/system questions, or take-home tasks depending on the role. If this is a university, it should include course fit, financial aid, admission documents, essay/interview stories, and student or alumni connection checks.";
  }
